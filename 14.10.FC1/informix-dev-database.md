...
*  ```latest```, ```14.10.UC1DE```,  ```12.10.UC12W1DE```,  ```12.10.UC11DE```  
...
### User Supplied ONCONFIG

* ```User Supplied ONCONFIG``` When a bind mount point is used with the __-v__ option, you can specify an __ONCONFIG__ to be used. Place a file named ```onconfig``` in the external storage volume directory on the host. 
...
RESIDENT=1
BUFFERPOOL=size=2k,buffers=250000,lrus=8,lru_min_dirty=50,lru_max_dirty=60
````
